<template>
    <div class='reg setpwd1 childPage' v-cloak>
        <head-top head-title='找回交易密码' :go-back='true'></head-top>
        ...
        methods: {
            ...
            validate () {
                if (!this.IDcard) {
                    ...
                    this.$http({
                        method: 'post',
                        url: this.HOST + '/user/findpaypwd.htm',
                        params: {
                            step: 1,
                            IDcard: that.IDcard,
                        ...
                        if (data.success) {
                            that.$router.push({path: "findTraderPwd/findTraderPwd1"});
                        } else {
            ...
            countDown (message) {
                if (!this.mobile) {
                    ...
                    this.$http({
                        method: 'post',
                        url: this.HOST + '/user/sendcheckcode.htm',
                        params: {
                            type: 2
                        }
                    }).then(function (response) {
